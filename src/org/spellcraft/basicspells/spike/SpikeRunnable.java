...
import org.spellcraft.basicspells.spike.AbstractSpikeSpell;

public class SpikeRunnable implements Runnable {

	private AbstractSpikeSpell caller;
	...
	public SpikeRunnable(AbstractSpikeSpell ncaller, SpikeAction naction,Block ntargetBlock)
	{
		caller = ncaller;
		action = naction;
		targetBlock = ntargetBlock;
	...
	{
		SpellCraft plugin = caller.getSpellCraft();
		if(canPlaceCactus(targetBlock))
		{
			...
			{
				plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new SpikeRunnable(targetBlock.getRelative(0, iii, 0)),b);
				b++; // Frequency set.
			}

			plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new SpikeRunnable(targetBlock,originalTargetMaterial,support),300); // Destroy cactus in 15 seconds.
		}
	}
