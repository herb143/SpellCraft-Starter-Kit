...
import org.spellcraft.basicspells.spike.SpikeRunnable;

public abstract class AbstractSpikeSpell extends TargetingSpell {
	...
	public AbstractSpikeSpell(Player playerinstance, SpellCraft instance, SpellType ispellType, int nrange, String iname,String idescription,ItemStack... irequirements)
	{
	...
	protected void castSpell(LivingEntity target)
	{
		...
		if(getDistance(targetBlock.getLocation(), player.getLocation()) <= range || targetBlock.getType() != Material.AIR) // If it's out of range, or air, they don't need to know.
		{
			...
			{
				plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new SpikeRunnable(this,SpikeRunnable.SpikeAction.SINGLE_CACTUS,targetBlocks[iii]),iii);
			}
		}
